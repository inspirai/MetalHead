...
class AMPLoader:
    ...
    def __init__(
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file, "r") as f:
                ...
                motion_data = np.array(motion_json["Frames"])  # (33, 61)
                # root_pos 3, root_rot 4, joint_pos 12, foot_pos 12, lin_vel 3, ang_vel 3, joint_vel 12, foot_vel 12
                # [fl, fr, rl, rr]
                motion_data = self.reorder_from_pybullet_to_isaac(motion_data)  # 这里的motion capture是从pybullet中捕获的

                # Normalize and standardize quaternions.
                for f_i in range(motion_data.shape[0]):
                    root_rot = AMPLoader.get_root_rot(motion_data[f_i])  # (4,)
                ...
                # Remove first 7 observation dimensions and last 12 foot_vel (root_pos and root_orn and foot_vel). (33, 42)
                self.trajectories.append(torch.tensor(
                    motion_data[
                        :,
                        AMPLoader.ROOT_ROT_END_IDX:AMPLoader.JOINT_VEL_END_IDX
                    ], dtype=torch.float32, device=device))
                self.trajectories_full.append(torch.tensor(  # (33, 49)
        ...
        if self.preload_transitions:
            ...
            # root_pos [:3], root_rot [3:7], joint_pos [7:19], foot_pos [19:31], lin_vel [31:34], ang_vel [34:37], joint_vel [37:49]
    ...
    def reorder_from_pybullet_to_isaac(self, motion_data):
        ...
        """
        root_pos = AMPLoader.get_root_pos_batch(motion_data)  # (33, 3)
        root_rot = AMPLoader.get_root_rot_batch(motion_data)  # (33, 4)  # (i, j, k, w)
        ...
        lin_vel = AMPLoader.get_linear_vel_batch(motion_data)  # (33, 3)
        ang_vel = AMPLoader.get_angular_vel_batch(motion_data)  # (33, 3)
        ...
        foot_vel = np.hstack([fv_fl, fv_fr, fv_rl, fv_rr])  # (33, 12)
